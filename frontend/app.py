...
async def handle_first_question(combined_input) -> None:
    ...
    session_first_question = {
        "chat_history": [
            ChatCompletionUserMessageParam(role="user", content=combined_input)
        ],
...
async def handle_single_step_ai_response(ai_response: dict) -> None:
    ...
    if not ai_response or ai_response.get("type") == "error":
        st.warning(
            "AIとの通信に問題が発生しました。APIキーや入力内容を確認し、もう一度お試しください。"
        )
...
def _handle_questions_response(ai_response: dict) -> None:
    ...
    if st.session_state.question_counter >= 5:
        ...
        st.session_state.dx_solution = {
            "tools": ai_response.get(
                "tools", [ai_response.get("tool", "利用可能な最適なDXツール")]
            ),
            "primary_tool": ai_response.get(
                "primary_tool", ai_response.get("tool", "利用可能な最適なDXツール")
            ),
            "tool_combinations": ai_response.get(
                "tool_combinations",
                [
                    {
                        "tool": ai_response.get("tool", "利用可能な最適なDXツール"),
                        "purpose": "主要な解決手段",
                        "todos": ai_response.get(
                            "todos", ["現時点で考えられる最適なToDo"]
                        ),
                    }
                ],
            ),
            "todos": ai_response.get("todos", ["現時点で考えられる最適なToDo"]),
            "message": "質問回数の制限に達したため、限られた情報に基づく提案となっています: "
            + ai_response["message"],
        }
        ...
        for q_text in st.session_state.ai_questions:
            st.session_state.chat_history.append(
                ChatCompletionAssistantMessageParam(
                    role="assistant", content=f"質問: {q_text}"
                )
            )
...
async def handle_user_answers_form() -> None:
    ...
    for i, question_text in enumerate(st.session_state.ai_questions):
        temp_answers[question_text] = st.text_area(
            question_text, key=f"answer_q_openai_{i}", height=100
        )
...
def _process_user_answers(temp_answers: dict) -> None:
    ...
    for q_text, ans_text in temp_answers.items():
        ...
        user_responses_for_history.append(
            ChatCompletionUserMessageParam(
                role="user", content=f"(質問「{q_text}」への回答) {ans_text}"
            )
        )
...
def _update_session_with_answers(
    temp_answers: dict, user_responses_for_history: list
) -> None:
    """
...
def update_checkbox_state_descriptions():
    st.session_state.use_tools_and_descriptions = (
        st.session_state.use_tools_and_descriptions_key
    )


def update_checkbox_state_llms():
    st.session_state.use_multiple_system_prompts = (
        st.session_state.use_multiple_system_prompts_key
    )
...
async def main():
    ...
    st.caption(
        "ふわっとした「やりたいこと」から、DXのテーマとToDoを具体化します。(OpenAI API連携版)"
    )
    await state_init()
    ...
    if st.session_state.use_tools_and_descriptions:
        st.sidebar.checkbox(
            "複数のシステムプロンプトを利用して追加の質問が必要か判断させる",
    ...
    st.caption(
        "現状の業務やデータの詳細を理解するために、関連ファイルをアップロードできます。"
    )
    ...
    with st.expander("ファイルをアップロードする", expanded=True):
        ...
        if uploaded_file:
            ...
            with st.spinner(f"ファイル「{uploaded_file.name}」を処理中..."):
                ...
                st.success(
                    f"ファイル「{uploaded_file.name}」が正常にアップロードされました。"
                )
    ...
    # 送信ボタン
    if st.button("AIに相談する", key="submit_initial_request_openai"):
        if initial_user_request:
            ...
            st.session_state.user_request_ = (
                initial_user_request  # 元のユーザー入力だけを保持
            )
    ...
    if st.session_state.conversation_stage == PromptType.DECISION:
        with st.spinner("AIが分析中です...しばらくお待ちください。"):
            ...
            if st.session_state.use_multiple_system_prompts:
                ...
                prompt_type = None
            ai_response = await fetch_dx_tool_suggestions(
    ...
    if (
        st.session_state.conversation_stage == PromptType.QUESTION
        and st.session_state.question_counter <= MAX_QUESTION_ROUNDS
    ):
        # 残りの質問回数を表示
        ai_response = await fetch_dx_tool_suggestions(
            st.session_state.chat_history,
        ...
        st.info(
            f"AIによる質問回数: {st.session_state.question_counter}回 / 最大{MAX_QUESTION_ROUNDS}回中"
        )

        # AIの質問を促すメッセージを表示
        _handle_questions_response(
            ai_response
        )  # This function now sets ai_questions and updates conversation_stage
        st.info(
            st.session_state.chat_history[-1 - len(st.session_state.ai_questions)][
                "content"
            ]
        )
    ...
    if (
        st.session_state.conversation_stage == PromptType.SOLUTION
        or st.session_state.question_counter > MAX_QUESTION_ROUNDS
    ):
        ai_response = await fetch_dx_tool_suggestions(
            st.session_state.chat_history,
            st.session_state.use_tools_and_descriptions,
    ...
    if (
        st.session_state.conversation_stage == "SHOWING_SOLUTION"
        and st.session_state.dx_solution
    ):
        ...
        # 主要なDXツールの表示
        primary_tool = solution.get(
            "primary_tool",
            solution.get("tools", ["N/A"])[0] if solution.get("tools") else "N/A",
        )
        ...
        if "tool_combinations" in solution and solution["tool_combinations"]:
            ...
            for i, tool_combo in enumerate(solution["tool_combinations"]):
                with st.expander(
                    f"{i + 1}. {tool_combo.get('tool', 'N/A')} - {tool_combo.get('purpose', '')}"
                ):
    ...
    if st.sidebar.checkbox(
        "チャット履歴を表示する", key="show_chat_history_openai", value=True
    ):
